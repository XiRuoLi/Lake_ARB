...
# Loading data
file_path = 'ML_table.xlsx'
data = pd.read_excel(file_path)

# Defining X and y
X = data.iloc[:, 1:6]
y_columns = data.columns[7:8]
...
param_grids = {
    'XGBoost': {
        'xgboost__n_estimators': [10, 20, 50, 100],
        'xgboost__max_depth': [1, 3, 5],
        'xgboost__learning_rate': [0.01, 0.05],
        'xgboost__reg_alpha': [1, 5, 10],
        'xgboost__reg_lambda': [0.1, 1, 10]
    },
    'Random Forest': {
        'randomforest__n_estimators': [50, 100, 200, 300, 400, 500],
        'randomforest__max_depth': [1, 2, 3, 5, 10, 20],
        'randomforest__min_samples_split': [0.1, 1, 2, 5]
    },
    'K Nearest Neighbors': {
...
for y_column in y_columns:
    ...
    for model_name, model in models.items():
        ...
        # Obtaining the best combination of hyperparameters
        ...
        # Evaluating with overall dataset
        ...
        # If XGBoost, extract feature importances
        if model_name == 'XGBoost':
            best_xgb_model = grid_search.best_estimator_.named_steps['xgboost']
